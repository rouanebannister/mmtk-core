...
use crate::scheduler::{GCWork, GCWorker}; //add. Needs more scheduling to make sure nursery collection works right
use crate::util::alloc::{Allocator, BumpAllocator};
use crate::util::forwarding_word;
use crate::util::{Address, ObjectReference, OpaquePointer};
use crate::vm::*; //change: from vm::VMBinding
use crate::MMTK;
...
impl<VM: VMBinding> CopyContext for GenCopyCopyContext<VM> {
    ...
    fn new(mmtk: &'static MMTK<Self::VM>) -> Self {
        Self {
            plan: unsafe { &*(&mmtk.plan as *const _ as *const GenCopy<VM>) },//change
...
//Add: This is a new structure for cleaning specifically the nursery
#[derive(Default)]
...
// Change: This now does a full heap collection.
#[derive(Default)]
